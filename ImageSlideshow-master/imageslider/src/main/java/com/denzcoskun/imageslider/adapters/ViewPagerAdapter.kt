...
class ViewPagerAdapter(context: Context?,
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): View{
        var currentPos = position % imageList?.size!!
        ...
        if (imageList!![currentPos].title != null){
            textView.text = imageList!![currentPos].title
        ...
        val loader = if (imageList!![currentPos].imageUrl == null){
            Picasso.get().load(imageList!![currentPos].imagePath!!)
        }else{
            Picasso.get().load(imageList!![currentPos].imageUrl!!)
        }

        // set Picasso options.
        if ((scaleType != null && scaleType == ScaleTypes.CENTER_CROP) || imageList!![currentPos].scaleType == ScaleTypes.CENTER_CROP){
            loader.fit().centerCrop()
        } else if((scaleType != null && scaleType == ScaleTypes.CENTER_INSIDE) || imageList!![currentPos].scaleType == ScaleTypes.CENTER_INSIDE){
            loader.fit().centerInside()
        }else if((scaleType != null && scaleType == ScaleTypes.FIT) || imageList!![currentPos].scaleType == ScaleTypes.FIT){
            loader.fit()
        }
        ...
        imageView.setOnClickListener {
            ...
            when {
                currentTouchTime - lastTouchTime < 250 -> {
                    itemClickListener?.doubleClick(currentPos)
                }
                else -> {
                    itemClickListener?.onItemSelected(currentPos)
                }
            }
        }
